...
type AdgroupTargeting struct {
	...
	TargetingType  uint   `json:"targeting_type,omitempty" codec:",omitempty"` // 定向类型
	TargetingValue string `json:"targeting_value" codec:",omitempty"`          // 定向值，多个值以英文逗号分隔","
	BidType        uint8  `json:"bid_type,omitempty" codec:",omitempty"`       // 价格出价类型：1.基础出价 2.溢价
	Price          uint   `json:"price" codec:",omitempty"`                    // 定向出价或者溢价价格，单位是分
